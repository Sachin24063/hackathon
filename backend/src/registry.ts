...
export interface Tool {
  ...
  description: string;
  department: string;
  requiredPermissions: string[]; // e.g., ['Employee', 'Manager', 'HR', 'Finance', 'IT_Admin', 'Sales', 'Support']
  inputSchema: {
    [paramName: string]: ToolParameter;
  };
  outputSchema: any;
  mockHandler: (params: any, context: { userRole: string; userId: string; userName: string; db: any }) => any;
}
...
export const toolsRegistry: Tool[] = [
  ...
  },
  {
    ...
    mockHandler: (params, ctx) => {
      ...
      const type = (params.leaveType || 'casual').toLowerCase() as 'casual' | 'sick' | 'annual';
  ...
  },
  {
    ...
    mockHandler: (params, ctx) => {
      ...
      const reqIndex = requests.findIndex((r: any) => r.requestId === params.requestId);
      ...
      // Restore balance
      const type = req.leaveType as 'casual' | 'sick' | 'annual';
  ...
  },
  {
    ...
    mockHandler: (params, ctx) => {
      const budgets: { [key: string]: { allocated: number; spent: number } } = {
