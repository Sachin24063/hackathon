...
export interface UnifiedRouterOutput {
  selected_tools: string[];
  plan: string[];
  clarify: boolean;
  clarify_question: string | null;
  
  // IEA Specific Extensions
  department: string;
  confidence: number;
...
export class TSIntelligentRouter {
  private catalog: any;
  private tools: any[];
  private toolById: Record<string, any>;
  private nearDuplicateGroups: string[][];
  ...
  private initializeTfidf() {
    ...
    for (const t of this.tools) {
      ...
      const uniqueTokens = new Set((tText.match(/[a-z0-9]+/g) || []) as string[]);
  ...
  private resolveToolId(targetId: string): string | null {
    ...
    if (targetId === 'bi.generate_chart' && !this.toolById['bi.generate_chart']) {
      if (this.toolById['bi.create_visualization']) return 'bi.create_visualization';
    }
    if (targetId === 'comm.send_slack_message' && !this.toolById['comm.send_slack_message']) {
      if (this.toolById['comm.post_slack_message']) return 'comm.post_slack_message';
    }

    return null;
  }

  public route(query: string): UnifiedRouterOutput {
    ...
    let convResponse: string | null = null;
    ...
    for (const sig of SIGNATURES) {
      ...
      let planResolved: string[] = [];
      if (sig.plan) {
        for (const tid of sig.plan) {
          const res = this.resolveToolId(tid);
          if (res) planResolved.push(res);
        }
      }
    ...
    const tokens = ((qLower.match(/[a-z0-9]+/g) || []) as string[]).filter(tok => !STOPWORDS.has(tok));
    if (tokens.length === 0) {
      return {
        selected_tools: [],
        plan: [],
    ...
    const rawTokensSetForCluster = new Set((qLower.match(/[a-z0-9]+/g) || []) as string[]);
    for (const t of this.tools) {
      const cluster = t.cluster || '';
      if (cluster) {
        const isClusterNameMatch = rawTokensSetForCluster.has(cluster.toLowerCase()) || 
                                   cluster.toLowerCase().split(/[_\.]/).some((sub: string) => rawTokensSetForCluster.has(sub));
        const tTokensSet = new Set(((t.name + " " + (t.description || "")).toLowerCase().match(/[a-z0-9]+/g) || []) as string[]);
    ...
    const rawTokensSet = new Set((qLower.match(/[a-z0-9]+/g) || []) as string[]);
    const scoredTools: { tool: any; score: number }[] = [];
    for (const t of this.tools) {
      const tText = `${t.name} ${t.description} ${(t.tags || []).join(' ')}`.toLowerCase();
      const tTokens = (tText.match(/[a-z0-9]+/g) || []) as string[];
      ...
      if (t.tags) {
        for (const tag of t.tags) {
          if (rawTokensSet.has(tag.toLowerCase())) score += 2;
        }
      }
    ...
    scoredTools.sort((a, b) => {
      ...
      const getRoleRank = (permissions: string[] = []) => {
        if (permissions.length === 0) return 0;
        if (permissions.includes('Employee')) return 1;
        if (permissions.includes('Manager')) return 2;
        return 3;
      };
      ...
      const getSafetyRank = (effects: string = 'read') => {
        if (effects === 'read') return 1;
        if (effects === 'write') return 2;
        return 3;
      };
